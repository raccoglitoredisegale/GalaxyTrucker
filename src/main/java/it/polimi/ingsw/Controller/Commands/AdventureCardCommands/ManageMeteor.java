...
public class ManageMeteor implements ServerControllerCommand {
    @Override
    public void execute(ClientHandler handler, Message message) {
        ...
        try {
            ...
            msg.MessageMeteorSwarm(handler.getPlayer(), ((MeteorCardMessage) message).getCanDefend(), ((MeteorCardMessage) message).getRemovedComponentFrom(), ((MeteorCardMessage) message).getRemovedBatteryFrom());
            ApplyEffect applyEffect = new ApplyEffect();
            applyEffect.visit(meteorSwarm, msg);
            gc.setAffectedPlayers(handler.getPlayer());
            handler.sendToClient(new Message(MessageType.MANAGE_METEOR_OK));
            if (gc.getAffectedPlayers().size() == gc.getInGamePlayers().size() && meteorSwarm.getCurrentMeteors().isEmpty()) {
                ...
                gc.checkGameEnded();
            } else if (gc.getAffectedPlayers().size() == gc.getInGamePlayers().size() && !meteorSwarm.getCurrentMeteors().isEmpty()) {
                meteorSwarm.setAlreadyShown(true);
                ContinueAdventureCardMessage serverResponse = new ContinueAdventureCardMessage(MessageType.CONTINUE_ADVENTURE_CARD, meteorSwarm, gc.getCurrentPlayer());
            ...
            handler.sendToClient(new Message(MessageType.MANAGE_METEOR_KO));
        }
    }
}
