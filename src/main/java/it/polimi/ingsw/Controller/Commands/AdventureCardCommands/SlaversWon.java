...
public class SlaversWon implements ServerControllerCommand {
    @Override
    public void execute(ClientHandler handler, Message message) {
        ...
        try {
            handler.getPlayer().getShip().acceptRemoveBattery(applyOperation, ((SlaversWonCardMessage) message).getRemovedBatteryFrom());
            handler.sendToClient(new Message(MessageType.SLAVERS_WON_OK));
        } catch (Exception e) {
            handler.sendToClient(new Message(MessageType.SLAVERS_WON_KO));
            gc.setCurrentPlayer();
