...
public class HandleEpidemicCommand implements ClientCommand {
    @Override
    public void execute(Client client, String command) {
        ...
        try {
            if (command.equals("apply")) {
                ...
                Message message = new Message(MessageType.APPLY_EPIDEMIC);
                Message response = client.sendMessageAndWaitResponse(message, getCreateLobbyTypesMessage());
                if (response.getType() == MessageType.APPLY_EPIDEMIC_OK) {
                    tui.print("epidemic has been applied");
                    ...
                    tui.print("Error applying epidemic");
                }
            } else tui.print("Syntax error for command: " + command);
        } catch (Exception e) {
            tui.print("Error occurred while handling epidemic");
    ...
    public ArrayList<MessageType> getCreateLobbyTypesMessage() {
        return new ArrayList<MessageType>(
                Arrays.asList(MessageType.APPLY_EPIDEMIC_OK, MessageType.APPLY_EPIDEMIC_KO)
        );
    }
